...
.PHONY: measurements-rbuilder
measurements-rbuilder: measurements-image image-rbuilder ### Generates measurements for rbuilder image. The measurements can be found in reproducible-build/artifacts-rbuilder/measurement-<image>.json.
	chmod 0777 $(REPRODUCIBLE_BUILD_DIR)/artifacts-rbuilder
	$(DOCKER) run --rm \
		-v $(REPRODUCIBLE_BUILD_DIR)/artifacts-rbuilder:/artifacts \
		-v $(BASE_BUILD_DIR)/rbuilder:/build \
		yocto-measurements:latest
	chmod 0755 $(REPRODUCIBLE_BUILD_DIR)/artifacts-rbuilder
